...
var Node = module.exports = Peer.extend({
  ...
  initialize: function(id, options) {
    ...
    // provide persistence config for later value management instantiation
    this.config.value_management = this.config.value_management||{};
    this.config.value_management['persistenceConfig'] = this.config.persistence||{type:'memory'};
          ...
          // else launch an iterativeFindValue
            return this.iterativeFindValue(key)
                       .pipe(function(kv) {
                          ...
                          // object with key/value: pipe the key
                          return kv.value;
                        });
          }, this)
  ...
  findNode: function(id, callback, context){

    context = context || this;

    this.iterativeFindNode(id).then(
      function(peers) {
        if (callback) {

          for (var i=0; i < peers.array.length; i++){
            var peer = peers.array[i];
            if (peer._id == id){
              callback.call(context, peer);
              break;
            }
          }
        }
  ...
  ping : function(address, id, callback, context) {
    context = context || this;
    var peer = new Peer(address, id);
    console.log(peer);
    var ping = new PingRPC(peer);
 ...
 //# INTERNAL EVENTS HANDLING
  ...
  routingTableEvents : {
     ...
     * peers for a REFRESH_TIMEOUT time), do an iterative find node
